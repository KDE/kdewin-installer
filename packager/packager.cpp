    ...
    if (m_special)
        switch (type) {
            case BIN:
                ...
                return true;
            case DEBUG:
                if (m_type.contains("vc"))
                {
                    generateFileList(fileList, dir, ".", "*.pdb");
                }
                else if (m_type.contains("mingw"))
                {
                    generateFileList(fileList, dir, ".", "*.sym");
                }
                generateFileList(fileList, dir, "bin",  "*d4.dll *d.dll", "");
    ...
    else
    {
        switch (type) {
            ...
            case DBG:
                ...
                if (m_type.startsWith("vc") || m_type.startsWith("x64-vc"))
                {
                    generateFileList(fileList, dir, ".", "*.pdb");
                }
    ...
    Q_FOREACH(const InstallFile &file, fileList) {
        ...
        if(!fn.endsWith('/')) 
        {
            ...
            if (m_checkSumMode.isEmpty())
                continue;
            HashFile hf(m_checkSumMode == "sha1" ? HashFile::SHA1 : HashFile::MD5, file, rootDir);
